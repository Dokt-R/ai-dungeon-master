...
class TestAdminCog:
    ...
    async def test_on_member_join_success(
        self, admin_cog_with_mock_client, mock_member
    ):
        ...
        cog.api_client.set_response_override(
            "create_player",
            {"player_id": "12345", "username": "TestUser", "status": "created"},
        )
    ...
    async def test_on_member_join_error_handling(
        self, admin_cog_with_mock_client, mock_member
    ):
    ...
    async def test_server_setup_success(
        self, admin_cog_with_mock_client, mock_interaction
    ):
    ...
    async def test_server_setup_permission_denied(
        self, admin_cog_with_mock_client, mock_interaction
    ):
    ...
    async def test_server_setkey_success(
        ...
        await cog.server_setkey_.callback(
            cog, mock_interaction, "sk-test-api-key-12345"
        )
    ...
    async def test_server_setkey_permission_denied(
        self, admin_cog_with_mock_client, mock_interaction
    ):
    ...
    async def test_server_setkey_api_error(
        self, admin_cog_with_mock_client, mock_interaction
    ):
        ...
        cog.api_client.set_exception_override(
            "set_server_config", ValidationError("EMPTY_API_KEY")
        )
    ...
    async def test_manage_guild_permission_sufficient(
        self, admin_cog_with_mock_client, mock_interaction
    ):
    ...
    async def test_api_client_call_tracking(
        self, admin_cog_with_mock_client, mock_member, mock_interaction
    ):
