...
async def test_character_add_command_validation_error(mock_character_cog):
    ...
    cog.api_client.set_exception_override('add_character', ApiValidationError(ErrorCode.VALIDATION_ERROR))
...
async def test_character_update_command_validation_error(mock_character_cog):
    ...
    from packages.shared.exceptions import ValidationError as ApiValidationError
    cog.api_client.set_exception_override('update_character', ApiValidationError(ErrorCode.UNKNOWN))
...
async def test_character_remove_command_not_found_error(mock_character_cog):
    ...
    mock_character_cog.api_client.set_exception_override('remove_character', ApiNotFoundError(ErrorCode.CHARACTER_NOT_FOUND))

    await mock_character_cog.remove.callback(mock_character_cog, interaction, character_id=999)
    interaction.response.send_message.assert_awaited_once_with(
...
async def test_character_list_command_validation_error(mock_character_cog):
    ...
    mock_character_cog.api_client.return_value.__aenter__.return_value.post.return_value = (
        mock_response
    )
...
async def test_character_add_command_http_error(mock_character_cog):
    ...
    mock_character_cog
    mock_character_cog.api_client.set_exception_override('add_character', Exception("Network error"))

    await mock_character_cog.add.callback(mock_character_cog, interaction, name="Hero", character_url=None)
...
async def test_character_update_command_http_error(mock_character_cog):
    ...
    mock_character_cog.api_client.set_exception_override('set_server_config', CustomException())
    
    await mock_character_cog.update.callback(
        mock_character_cog, interaction, character_id=1, name="NewName", character_url=None
    )
    interaction.response.send_message.assert_awaited_once()
    args, kwargs = interaction.response.send_message.call_args
    assert ErrorCode.UNKNOWN.player_message.lower() == args[0].lower()
...
async def test_character_remove_command_http_error(mock_character_cog):
    ...
    await mock_character_cog.remove.callback(mock_character_cog, interaction, character_id=1)
    interaction.response.send_message.assert_awaited_once()
    args, kwargs = interaction.response.send_message.call_args
    assert ErrorCode.UNKNOWN.player_message.lower() == args[0].lower()
    assert kwargs.get("ephemeral") is True
