...
class TestCharacterCog:
    ...
    async def test_add_character_success(
        self, character_cog_with_mock_client, mock_interaction
    ):
        ...
        cog.api_client.set_response_override(
            "add_character",
            {"character_id": 123, "name": "Test Character", "player_id": "12345"},
        )
    ...
    async def test_add_character_duplicate_error(
        self, character_cog_with_mock_client, mock_interaction
    ):
    ...
    async def test_update_character_success(
        ...
        cog.api_client.set_response_override(
            "update_character",
            {"character_id": 123, "name": "Updated Character", "player_id": "12345"},
        )
        ...
        mock_interaction.response.send_message.assert_awaited_once_with(
            "Character updated successfully.", ephemeral=True
        )
    ...
    async def test_update_character_no_fields_error(
        self, character_cog_with_mock_client, mock_interaction
    ):
    ...
    async def test_update_character_not_found(
        ...
        cog.api_client.set_exception_override(
            "update_character", NotFoundError("CHARACTER_NOT_FOUND")
        )

        with pytest.raises(NotFoundError):
            await cog._handle_update(mock_interaction, 999, "New Name", None)

    async def test_remove_character_success(
        self, character_cog_with_mock_client, mock_interaction
    ):
        ...
        # Verify the interaction response
        mock_interaction.response.send_message.assert_awaited_once_with(
            "Character removed successfully.", ephemeral=True
        )
    ...
    async def test_remove_character_not_found(
        ...
        # Set up mock to raise not found error
        cog.api_client.set_exception_override(
            "remove_character", NotFoundError("CHARACTER_NOT_FOUND")
        )
    ...
    async def test_api_client_call_tracking(
        ...
        await cog._handle_character_add(
            mock_interaction, "Test 2", "http://example.com"
        )
