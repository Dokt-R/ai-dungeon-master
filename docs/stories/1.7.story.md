...
## Tasks / Subtasks
- [ ] Implement `/campaign new [name]` command in `campaign_cog.py` to create a new campaign and persist it in `data/campaigns/[campaign_name]/`. After creation, prompt the user for character setup (digital or physical sheet).  
  (AC: 1, 10)
- [ ] Implement `/campaign join [name]` command to allow players to join an existing campaign. Prompt for character setup.  
  (AC: 2, 10, 12)
- [ ] Implement `/campaign continue` command to resume the last active campaign and load the last clean save point. If user was disconnected, restore from autosave.  
  (AC: 3, 11)
- [ ] Display explicit state transition message on campaign start/continue.  
  (AC: 4)
- [ ] Implement `/campaign end` command to autosave and end the current session.  
  (AC: 5)
