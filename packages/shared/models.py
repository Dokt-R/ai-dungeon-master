...
class ActionRequest(BaseModel):
    ...
    prompt: str = PydanticField(
        ...
        examples=["I want to investigate the strange statue", "I attack the goblin with my sword"]
    )
...
class ActionResponse(BaseModel):
    ...
    narrative: str = PydanticField(
        ...
        examples=["You carefully examine the statue, noticing intricate carvings that seem to tell a story..."]
    )
...
class SRDCompliance(BaseModel):
    ...
    license_version: str = PydanticField(
        ...,
        description="SRD license version",
        examples=["5.1", "5.0"]
    )
    ...
    last_verified: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="Last compliance verification date"
    )
...
class Monster(BaseModel):
    ...
    monster_name: str = PydanticField(
        ...,
        description="Official monster name",
        examples=["Goblin", "Orc", "Dragon"]
    )
    ...
    actions: Optional[str] = PydanticField(
        ...
        examples=["Scimitar. Melee Weapon Attack: +4 to hit, reach 5 ft., one target. Hit: 5 (1d6 + 2) slashing damage."]
    )

    special_abilities: Optional[str] = PydanticField(
        None,
        description="Special abilities and traits",
        examples=["Nimble Escape. The goblin can take the Disengage or Hide action as a bonus action on each of its turns."]
    )

    description: Optional[str] = PydanticField(
        None,
        description="Monster description and background",
        examples=["Goblins are small, green-skinned humanoids with a mischievous and cruel nature."]
    )
    ...
    is_active: bool = PydanticField(
        default=True,
        description="Whether this monster is active and available"
    )
...
class Spell(BaseModel):
    ...
    spell_id: Optional[int] = PydanticField(
        None,
    ...
    duration: str = PydanticField(
        ...
        examples=["Instantaneous", "Concentration, up to 1 minute", "1 hour", "Until dispelled"]
    )

    description: str = PydanticField(
        ...,
        description="Complete spell description and mechanics",
        examples=["A bright streak flashes from your pointing finger to a point you choose within range..."]
    )

    at_higher_levels: Optional[str] = PydanticField(
        None,
        description="Effects when cast at higher levels",
        examples=["When you cast this spell using a spell slot of 4th level or higher, the damage increases by 1d6 for each slot level above 3rd."]
    )
    ...
    is_active: bool = PydanticField(
        default=True,
        description="Whether this spell is active and available"
    )
...
class Weapon(BaseModel):
    ...
    weight: str = PydanticField(
        ...
        examples=["3 lb.", "2 lb.", "4 lb."]
    )

    properties: List[str] = PydanticField(
        default_factory=list,
        description="Weapon properties",
        examples=[["Versatile (1d10)"], ["Ammunition (range 80/320)"], ["Light", "Finesse"]]
    )
    ...
    is_active: bool = PydanticField(
        default=True,
        description="Whether this weapon is active and available"
    )
...
class MemoryEvent(BaseModel):
    ...
    participants: List[str] = PydanticField(
        ...,
        description="Characters/NPCs involved"
    )
    ...
    version: int = PydanticField(
        default=1,
        description="Memory version for updates"
    )
...
class MemoryFact(BaseModel):
    ...
    fact_type: Literal["npc", "location", "quest", "relationship", "knowledge"] = PydanticField(
        ...,
        description="Type of fact"
    )

    subject: str = PydanticField(
        ...,
        description="Primary subject of the fact"
    )
    ...
    last_updated: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="When fact was last updated"
    )
...
class MemoryContext(BaseModel):
    ...
    summary: str = PydanticField(
        ...,
        description="Condensed memory summary for AI"
    )

    context_size: int = PydanticField(
        ...,
...
class MemoryOperation(BaseModel):
    ...
    memory_id: str = PydanticField(
        ...,
        description="ID of affected memory"
    )

    memory_type: str = PydanticField(
        ...,
        description="Type of memory affected"
    )
...
class UpdateMemoryFactRequest(BaseModel):
    ...
    related_events: Optional[List[str]] = PydanticField(
        None,
        description="Updated related events"
    )
...
class VoiceChannelInfo(BaseModel):
    ...
    user_limit: Optional[int] = PydanticField(
        None,
        description="User limit for channel",
        ge=0,
        le=99
    )
    ...
    created_at: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="When this info was retrieved"
    )
...
class VoiceConnection(BaseModel):
    ...
    channel_id: str = PydanticField(
        ...,
        description="Connected voice channel ID",
        pattern=r'^[0-9]+$'
    )
    ...
    participants: List[str] = PydanticField(
        default_factory=list,
        description="User IDs in voice"
    )

    last_activity: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="Last voice activity"
    )
...
class VoicePermission(BaseModel):
    ...
    user_id: str = PydanticField(
        ...,
        description="User or bot ID",
        pattern=r'^[0-9]+$'
    )

    can_connect: bool = PydanticField(
        ...,
        description="Can connect to voice channel"
    )

    can_speak: bool = PydanticField(
        ...,
        description="Can speak in voice channel"
    )

    can_mute_members: bool = PydanticField(
        ...,
...
class AudioStreamInfo(BaseModel):
    ...
    user_id: str = PydanticField(
        ...,
        description="User ID of the stream source",
        pattern=r'^[0-9]+$'
    )
    ...
    format: str = PydanticField(
        ...,
        description="Audio format (e.g., 's16le', 'opus')"
    )
    ...
    last_activity: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="Last audio activity"
    )
    ...
    volume_level: float = PydanticField(
        ...,
        description="Current volume level (0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
...
class VoiceSession(BaseModel):
    ...
    ended_at: Optional[datetime] = PydanticField(
        None,
    ...
    max_concurrent_participants: int = PydanticField(
        0,
        description="Maximum concurrent participants",
        ge=0
    )

    total_audio_time: float = PydanticField(
        0.0,
        description="Total audio time in seconds",
        ge=0.0
    )

    connection_issues: List[Dict[str, Any]] = PydanticField(
        default_factory=list,
        description="Connection issues encountered"
    )
...
class VoiceJoinRequest(BaseModel):
    ...
    force: bool = PydanticField(
        False,
        description="Force join even if already connected"
    )

    self_deaf: bool = PydanticField(
        False,
        description="Join deafened"
    )

    self_mute: bool = PydanticField(
        False,
...
class VoiceLeaveRequest(BaseModel):
    ...
    user_id: str = PydanticField(
        ...,
        description="User requesting to leave",
        pattern=r'^[0-9]+$'
    )
...
class VoiceStatusResponse(BaseModel):
    ...
    channel_id: Optional[str] = PydanticField(
        None,
        description="Current voice channel ID"
    )

    channel_name: Optional[str] = PydanticField(
        None,
        description="Current voice channel name"
    )

    guild_id: Optional[str] = PydanticField(
        None,
        description="Current guild ID"
    )
...
class VoiceChannelResponse(BaseModel):
    ...
    member_count: int = PydanticField(
        0,
...
class VoiceLatencyMetrics(BaseModel):
    ...
    metadata: Dict[str, Any] = PydanticField(
        default_factory=dict,
        description="Additional latency metadata"
    )
...
class AudioQualityMetrics(BaseModel):
    ...
    transcription_accuracy: Optional[float] = PydanticField(
        ...
        ge=0.0,
        le=1.0,
        description="STT accuracy score"
    )
    ...
    voice_clarity: Optional[float] = PydanticField(
        ...
        ge=0.0,
        le=5.0,
        description="Voice synthesis clarity rating"
    )

    audio_artifacts: int = PydanticField(
        default=0,
        ge=0,
        description="Number of audio artifacts detected"
    )
    ...
    noise_level: Optional[float] = PydanticField(
        ...
        ge=0.0,
        le=1.0,
        description="Background noise level (0.0 to 1.0)"
    )
    ...
    audio_format: Optional[str] = PydanticField(
        None,
...
class VoiceSessionConfig(BaseModel):
    ...
    max_concurrent_sessions: int = PydanticField(
        ...
        ge=1,
        le=100,
        description="Maximum concurrent voice sessions"
    )
    ...
    audio_retention_days: int = PydanticField(
        ...
        ge=0,
        le=365,
        description="Audio data retention period in days"
    )
    ...
    enable_quality_assessment: bool = PydanticField(
        default=True,
        description="Enable audio quality assessment"
    )
...
class PerformanceAlert(BaseModel):
    ...
    alert_type: Literal["latency", "quality", "error_rate", "resource_usage"] = PydanticField(
        ...,
    ...
    current_value: float = PydanticField(
        ...,
...
class PerformanceReport(BaseModel):
    ...
    time_range_end: datetime = PydanticField(
        ...,
        description="Report time range end"
    )
    ...
    average_latency: Optional[float] = PydanticField(
        None,
        description="Average total latency in seconds"
    )
    ...
    sessions_meeting_latency_target: int = PydanticField(
        default=0,
        description="Sessions meeting latency target (< 4s)"
    )

    average_quality_score: Optional[float] = PydanticField(
        None,
        description="Average audio quality score"
    )

    stt_success_rate: Optional[float] = PydanticField(
        None,
        description="STT success rate"
    )

    tts_success_rate: Optional[float] = PydanticField(
        None,
        description="TTS success rate"
    )

    error_rate: Optional[float] = PydanticField(
        None,
    ...
    alerts_generated: int = PydanticField(
        default=0,
        description="Number of alerts generated in period"
    )
...
class PrivacyComplianceRecord(BaseModel):
    ...
    data_collection_timestamp: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="When audio data was collected"
    )
    ...
    privacy_consent_obtained: bool = PydanticField(
        default=False,
        description="Whether user consent was obtained"
    )
    ...
    compliance_officer: str = PydanticField(
        default="system",
        description="Person responsible for compliance"
    )

    audit_trail: List[Dict[str, Any]] = PydanticField(
        default_factory=list,
        description="Audit trail of data access and processing"
    )
...
class VoiceCommandIntent(BaseModel):
    ...
    sentiment: Optional[float] = PydanticField(
        ...
        ge=-1.0,
        le=1.0,
        description="Text sentiment score (-1.0 to 1.0)"
    )
    ...
    urgency: Optional[float] = PydanticField(
        ...
        ge=0.0,
        le=1.0,
        description="Command urgency level"
    )

    created_at: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="When intent was created"
    )

    processing_time: float = PydanticField(
        ...,
        ge=0.0,
        description="Time taken to process intent in seconds"
    )
...
class CommandPattern(BaseModel):
    ...
    intent: str = PydanticField(
        ...,
        description="Associated intent type"
    )

    patterns: List[str] = PydanticField(
        ...,
        description="Text patterns to match"
    )
    ...
    context_requirements: Dict[str, Any] = PydanticField(
        default_factory=dict,
        description="Context requirements for this pattern"
    )
    ...
    usage_count: int = PydanticField(
        default=0,
        description="Number of times this pattern has been matched"
    )
...
class CommandHistory(BaseModel):
    ...
    speech_patterns: Dict[str, Any] = PydanticField(
        default_factory=dict,
        description="Learned speech patterns and preferences"
    )
...
class CommandExecutionResult(BaseModel):
    ...
    execution_id: str = PydanticField(
        ...,
    ...
    execution_time: float = PydanticField(
        ...,
        ge=0.0,
        description="Time taken to execute command in seconds"
    )

    response_text: Optional[str] = PydanticField(
        None,
        description="Response text to send back to user"
    )

    game_state_changes: Dict[str, Any] = PydanticField(
        default_factory=dict,
        description="Changes made to game state"
    )
    ...
    confidence_score: float = PydanticField(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence score of the command recognition"
    )
...
class CommandFeedback(BaseModel):
    ...
    categories: List[str] = PydanticField(
        ...
        examples=["command_recognition", "entity_extraction", "execution_speed", "response_clarity"]
    )

    suggested_improvement: Optional[str] = PydanticField(
        None,
        description="User's suggestion for improvement"
    )

    technical_details: Dict[str, Any] = PydanticField(
        default_factory=dict,
        description="Technical details for analysis"
    )
    ...
    resolved: bool = PydanticField(
        default=False,
        description="Whether feedback has been addressed"
    )

    resolution_notes: Optional[str] = PydanticField(
        None,
        description="Notes about how feedback was resolved"
    )
...
class AudioTranscriptionRequest(BaseModel):
    ...
    audio_data: bytes = PydanticField(
        ...,
        description="Raw audio data to transcribe"
    )
    ...
    provider: Optional[str] = PydanticField(
        "auto",
        description="Preferred STT provider"
    )
...
class TranscriptionResult(BaseModel):
    ...
    segments: Optional[List[Dict[str, Any]]] = PydanticField(
        None,
        description="Segment-level results"
    )
...
class SpeechSynthesisResult(BaseModel):
    ...
    audio_format: str = PydanticField(
        ...,
    ...
    voice_used: str = PydanticField(
        ...,
        description="Voice identifier used"
    )
...
class AudioProcessingConfig(BaseModel):
    ...
    silence_threshold: float = PydanticField(
        0.01,
        description="Threshold for silence detection (0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    noise_reduction: bool = PydanticField(
        True,
...
class AudioStreamInfo(BaseModel):
    ...
    total_transcriptions: int = PydanticField(
        0,
        description="Total transcriptions generated",
        ge=0
    )
...
class ProviderHealthStatus(BaseModel):
    ...
    total_requests: int = PydanticField(
        0,
...
class STTServiceStatus(BaseModel):
    ...
    queued_requests: int = PydanticField(
        0,
        description="Number of queued transcription requests",
        ge=0
    )
    ...
    last_activity: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="Last STT service activity"
    )
...
class TTSServiceStatus(BaseModel):
    ...
    active_syntheses: int = PydanticField(
        0,
        description="Number of active speech syntheses",
        ge=0
    )

    queued_requests: int = PydanticField(
        0,
        description="Number of queued synthesis requests",
        ge=0
    )
    ...
    last_activity: datetime = PydanticField(
        default_factory=datetime.utcnow,
        description="Last TTS service activity"
    )
