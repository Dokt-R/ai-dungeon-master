...
class StateAdapter:
    ...
    async def enrich_minimal_state(
        minimal_state: MinimalGameState, discord_user_id: str, discord_channel_id: str
    ) -> MinimalGameState:
    ...
    def update_from_action_result(
        current_state: MinimalGameState, action_result: Dict[str, Any]
    ) -> MinimalGameState:
        """Update MinimalGameState based on action results."""
        return MinimalGameState(**current_state, action_result=action_result)

    @staticmethod
    def update_from_parsed_intent(
        current_state: MinimalGameState, parsed_intent: Dict[str, Any]
    ) -> MinimalGameState:
        """Update MinimalGameState with parsed intent."""
        return MinimalGameState(**current_state, parsed_intent=parsed_intent)
    ...
    @staticmethod
    def prepare_state_for_logging(
        state: MinimalGameState, action_result: Dict[str, Any]
    ) -> Dict[str, Any]:
