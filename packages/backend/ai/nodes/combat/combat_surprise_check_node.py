...
async def surprise_check_node(state: ActionResolutionState) -> Dict[str, Any]:
    ...
    for participant_id, participant in participants.items():
        ...
        if perception_check.total < stealth_dc:
            ...
            print(f"---{participant['id']} IS SURPRISED (Perception: {perception_check.total} vs DC: {stealth_dc})---")
        else:
            participant["is_surprised"] = False
            print(f"---{participant['id']} IS NOT SURPRISED (Perception: {perception_check.total} vs DC: {stealth_dc})---")
