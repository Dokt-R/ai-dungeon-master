...
async def initiative_node_with_db(state: MinimalGameState) -> Dict[str, Any]:
    ...
    try:
        ...
        round_order = await game_service.update_initiative_order(
            state["campaign_id"], initiative_results
        )
        ...
        for i, char_id in enumerate(round_order, 1):
            ...
            result_lines.append(f"{i}. {char['name']} ({initiative_results[char_id]})")
...
async def combat_node_with_db(state: MinimalGameState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not campaign_context["in_combat"]:
            return {
                "action_result": "Not currently in combat. Use `/initiative` to start combat."
            }
        ...
        if current_char_id != state["character_id"]:
            ...
            return {"action_result": f"It's {current_char['name']}'s turn, not yours!"}
        ...
        campaign_chars = await game_service.get_campaign_characters(
            state["campaign_id"]
        )
        target = None
        ...
        return {"action_result": result, "dice_results": {"damage": damage}}
