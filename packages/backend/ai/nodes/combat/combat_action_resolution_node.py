...
from packages.shared.models.core_db_models import Character, Attack
...
async def resolve_combat_action_node(state: ActionResolutionState) -> Dict[str, Any]:
    ...
    engine = create_engine("sqlite:///./data/srd_database.sqlite") # Use your actual database URL
    with Session(engine) as session:
        game_state_service = GameStateService(session)
        
        combat_state = state["combat_state"]
        game_state = state["game_state"] # This is still the in-memory representation for now
        ...
        attacker_char = await game_state_service.get_character_by_id(active_participant_id)
        ...
        target_char = await game_state_service.get_character_by_name(target_name, campaign_id=game_state["campaign_id"]) # Assuming campaign_id is in game_state
        ...
        attack_bonus = attacker_char.dex_modifier + attacker_char.proficiency_bonus # Example: using dex for attack bonus
        ...
        if attack_roll_result.total >= defender_ac:
            ...
            if is_critical_hit:
                ...
                print(f"---CRITICAL HIT! {attacker_char.name} rolls with {actual_damage_dice} damage dice!---")
            ...
            print(f"---{attacker_char.name} HITS {target_char.name} for {damage_roll_result.total} damage! (Roll: {attack_roll_result.total} vs AC: {defender_ac})---")
            ...
            if target_char.hp <= 0:
                if target_char.player_id: # Check if it's a player character
                    ...
                    print(f"---{target_char.name} drops to 0 HP and is UNCONSCIOUS! They must make death saving throws.---")
                else:
            ...
            print(f"---{attacker_char.name} MISSES {target_char.name}! (Roll: {attack_roll_result.total} vs AC: {defender_ac})---")
        ...
        return {"combat_state": combat_state, "game_state": game_state, "action_result": {"type": "attack", "success": True, "target_name": target_char.name, "new_hp": target_char.hp, "defender_alive": target_char.is_alive, "target_id": target_char.character_id}}