...
async def end_turn_node(state: ActionResolutionState) -> Dict[str, Any]:
    ...
    player_alive = any(p["type"] == "player" and "dead" not in p["status_conditions"] for p in combat_state["participants"].values())
    enemies_alive = any(p["type"] == "enemy" and "dead" not in p["status_conditions"] for p in combat_state["participants"].values())
    ...
    current_index = combat_state["initiative_queue"].index(combat_state["active_turn_participant_id"])
    ...
    combat_state["active_turn_participant_id"] = combat_state["initiative_queue"][next_index]
