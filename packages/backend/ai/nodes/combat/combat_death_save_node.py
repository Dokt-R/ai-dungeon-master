...
async def death_save_node(state: ActionResolutionState) -> Dict[str, Any]:
    ...
    for participant_id, participant in combat_state["participants"].items():
        if (
            "unconscious" in participant["status_conditions"]
            and "dead" not in participant["status_conditions"]
        ):
            ...
            if roll >= 10:
                ...
                print(
                    f"---{participant_id} rolled a {roll} (Success)! Total successes: {participant['death_save_successes']}---"
                )
            else:
                participant["death_save_failures"] += 1
                print(
                    f"---{participant_id} rolled a {roll} (Failure)! Total failures: {participant['death_save_failures']}---"
                )
            ...
            if participant["death_save_successes"] >= 3:
                ...
                print(
                    f"---{participant_id} has failed three death saves and is DEAD!---"
                )
