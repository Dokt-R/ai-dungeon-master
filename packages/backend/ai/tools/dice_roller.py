...
class DiceRoller:
    ...
    def roll_with_advantage(self, sides: int = 20, advantage_type: AdvantageType = AdvantageType.NORMAL) -> Tuple[List[int], int]:
    ...
    def roll_ability_scores(self, method: str = "4d6_drop_lowest") -> Dict[str, int]:
        ...
        if method == "standard_array":
            ...
            return {ability: sum(self.roll_multiple_dice(3, 6)) for ability in abilities}
    ...
    def roll_hit_points(self, hit_die: int, level: int, constitution_modifier: int = 0) -> int:
    ...
    def roll_attack(self, attack_bonus: int, advantage_type: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
    ...
    def roll_saving_throw(self, save_bonus: int, advantage_type: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
        """Roll a saving throw with bonus and optional advantage/disadvantage."""
        roll = self.roll_dice_notation("1d20", advantage_type)
        roll.modifier = save_bonus
        roll.total = roll.rolls[0] if advantage_type == AdvantageType.NORMAL else max(roll.rolls) if advantage_type == AdvantageType.ADVANTAGE else min(roll.rolls)
        roll.total += save_bonus
        roll.description = f"Save: 1d20{'+' + str(save_bonus) if save_bonus >= 0 else str(save_bonus)}"
    ...
    def roll_skill_check(self, skill_bonus: int, advantage_type: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
