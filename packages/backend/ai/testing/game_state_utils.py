...
def create_example_game_state() -> GraphGameState:
    ...
    goblin_character = Character(
        ...
        charisma=8,
        level=1,
        copper_pieces=5, silver_pieces=0, electrum_pieces=0, gold_pieces=0, platinum_pieces=0,
        attacks=[
            Attack(
                name="Scimitar",
                bonus=4,
    ...
    player_dump['inventory'] = [item.model_dump_clean() for item in player_character.inventory]
    player_dump['attacks'] = [attack.model_dump_clean() for attack in player_character.attacks]

    goblin_dump = goblin_character.model_dump()
    goblin_dump['attacks'] = [attack.model_dump_clean() for attack in goblin_character.attacks]
...
async def test_action_resolution():
    ...
    test_actions = [
        ...
        {"action": "I attack the goblin with my sword", "expected_route": "combat_subgraph",
         "category": "Combat Demo", "test_exit_after_initiative": True},
    ...
    for field in ['player', 'campaign', 'inventory', 'attacks', 'action_history', 'updated_game_state']:
        ...
        try:
            ...
            if test_case.get("test_exit_after_initiative", False):
                ...
                if initiative_completed and exit_early:
                    ...
                    print(f"📖 Response: Initiative rolled successfully, system exited per test mode")

                    # Show combat state from result
                    combat_state = result.get('resolved_combat_state', {}) or test_state.get('combat_state', {})
                    if combat_state:
                    ...
                    print("⚠️  Expected initiative completion and exit_early, but didn't receive it")
            else:
            ...
            if updated_game_state and updated_game_state != game_state:
                ...
                for field in ['player', 'campaign', 'inventory', 'attacks', 'action_history']:
                    updated_player_dict.pop(field, None)
                updated_player_char = Character(**updated_player_dict)
                updated_player_char.inventory = [Item(**item_data) for item_data in updated_game_state['player']['inventory']]
                updated_player_char.attacks = [Attack(**attack_data) for attack_data in updated_game_state['player']['attacks']]
                ...
                original_player_char.inventory = [Item(**item_data) for item_data in game_state['player']['inventory']]
                original_player_char.attacks = [Attack(**attack_data) for attack_data in game_state['player']['attacks']]
                ...
                if (updated_player_char.copper_pieces != original_player_char.copper_pieces or
                    updated_player_char.silver_pieces != original_player_char.silver_pieces or
                    updated_player_char.electrum_pieces != original_player_char.electrum_pieces or
                    updated_player_char.gold_pieces != original_player_char.gold_pieces or
                    updated_player_char.platinum_pieces != original_player_char.platinum_pieces):
                    state_changes.append("Wallet changed.")

                if len(updated_player_char.inventory) != len(original_player_char.inventory):
                    state_changes.append(f"Inventory changed (count: {len(original_player_char.inventory)} → {len(updated_player_char.inventory)}).")
                else:
            ...
            if test_case.get("test_exit_after_initiative", False):
                ...
                if initiative_completed and exit_early:
                    ...
                    print("❌ EXIT_EARLY demo test failed - initiation may not have completed")
    ...
    print("🎯 Enhanced conditional routing implementation provides flexible action routing!")
