...
class GameStateService:
    ...
    async def character_to_combat_participant(self, participant_key: str, campaign_id: int) -> CombatParticipant:
    ...
    async def get_campaign_context(self, campaign_id: int) -> Dict[str, Any]:
        ...
        game_state = self.session.execute(
            select(CampaignGameState).where(CampaignGameState.campaign_id == campaign_id)
        ).scalars().first()
    ...
    async def get_character_by_name(self, name: str, campaign_id: Optional[int] = None) -> Optional[Character]:
    ...
    async def get_campaign_characters(self, campaign_id: int) -> List[Character]:
        ...
        return self.session.execute(select(Character).where(Character.campaign_id == campaign_id)).all()
    ...
    async def add_combat_participant(self, state: MinimalGameState, character: Character) -> Character:
        """Create a new character and add it to state."""
    ...
    async def delete_character(self, character_id: int):
        ...
        character = self.session.execute(select(Character).where(Character.character_id == character_id)).first()
        if character:
    ...
    async def get_campaign_state(self, campaign_id: int) -> Optional[CampaignGameState]:
        ...
        return self.session.execute(select(CampaignGameState).where(CampaignGameState.campaign_id == campaign_id)).first()

    async def update_campaign_state(self, minimal_state: MinimalGameState) -> MinimalGameState:
        """Update the campaign game state using MinimalGameState."""
        campaign_game_state = self.session.execute(
            select(CampaignGameState).where(CampaignGameState.campaign_id == minimal_state["campaign_id"])
        ).first()
        ...
        if not campaign_game_state:
            ...
            campaign_game_state.current_character_turn = minimal_state.get("character_id")
    ...
    async def log_action(self, campaign_id: int, character_id: Optional[int],
                         discord_user_id: str, discord_channel_id: str,
                         action_text: str, parsed_intent: Dict, result: str,
